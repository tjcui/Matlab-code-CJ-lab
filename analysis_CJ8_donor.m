...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
gmmodel                 = fitgmdist(donor,2);
gmel                    = find(gmmodel.mu==min(gmmodel.mu));
donorfit.mu          = gmmodel.mu(gmel(1));
donorfit.sigma       = sqrt(gmmodel.Sigma(gmel(1)));
        if threshold == 0
...
hold off;
            ...
            for ii=1:nregions
                ...
                fprintf(Cy3_fid_traI,'%d\n',ave_Cy3/totalnumber);
                fprintf(Cy5_fid_traI,'%d\n',ave_Cy5/totalnumber);
                fprintf(FRET_fid_lengthI,'%d %d\n',totalnumber-2,startpointtot(ii));
            end
...
fclose(fid_traceI_selected);
 ...
 if selection == 10,     % Auto-Threshold
     ...
     if isempty(timestart)
         ...
         figure(hdl1);
         subplot(2,1,1);
         h=plot(time,donor,'g',time,acceptor,'r', time, donor+acceptor+offset,'k',(timestart-1)*int_time,donor(timestart),'o',...
             (timestop-1)*int_time,donor(timestop),'o','linewidth',2);
         ...
         if ans=='x',
             ...
             for j=1:nregions,
                 ...
                 fprintf(Cy3_fid_traI,'%d\n',ave_Cy3/totalnumber);
                 fprintf(Cy5_fid_traI,'%d\n',ave_Cy5/totalnumber);
                 fprintf(FRET_fid_lengthI,'%d %d\n',totalnumber-2,timestart(j));       %apparently the L(j) element gives the starting point of the mouseclick in the standard version
                 %also note that the dwelltime here is the time between the two points,
             end % end of sized(2)
         end
