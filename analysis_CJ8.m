...
%   set(hdl1, 'Position',[0 scrsz(4)*0.5-70 scrsz(3)*0.75 scrsz(4)*0.5]);
   ...
   selection = menu('Analyze?', 'Next', 'Back', 'Go to...','Delete',...
       'Standard Analysis', 'Standard with Cross Correlation', 'Standard with Auto Correlation', 'Standard with Two-State Identification', 'End');
   ...
   if (selection > 4)&(selection <9), %Standard or Correlation analysis
...
%                fprintf(xoutfile,'%f\n',xCross(xcorr,2));
    ...
    input('Press any key to proceed ("x" to exit) --> ','s');
       if ans=='x',
          i= LD;
       end   
