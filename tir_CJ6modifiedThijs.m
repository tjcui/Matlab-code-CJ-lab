...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Modification by Thijs starts here
...
ddonor=donor(1,:);
if nnz(ddonor(ddonor>250))>(len/2)
    ddonor=(donor(2,:));
end

% bigthreshold = (max(ddonor)-min(ddonor))/4+ min(ddonor);

donorfit=fitdist(ddonor(ddonor)','normal');
back_donor= donorfit.mu;


%for the acceptor idem dito
aacceptor=acceptor(1,:);
if nnz(aacceptor(aacceptor>250))>(len/2)
    aacceptor=(acceptor(2,:));
end

acceptorfit=fitdist(aacceptor(aacceptor)','normal');
back_acceptor= acceptorfit.mu;
...
while ((Ntraces/2 - 1) > 0) & (i < Ntraces/2 - 2) ,
   ...
   for k = 1:len,
       if (donor(i,k)+acceptor(i,k)) < maxcps*0.05,
