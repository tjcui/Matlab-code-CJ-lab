...
%     for ii=2:length(timestart)
    ...
    %+1 and -1 for the donor so that we don't get these weird spikes at
    %times
    realtimestartacceptor = realtimestart;
    realtimestopacceptor = realtimestop;
    acceptor(realtimestartacceptor) =  acceptor(realtimestartacceptor)/1000;
    acceptor(realtimestopacceptor) =  acceptor(realtimestopacceptor)/1000;
    
    
    if length(realtimestart)==length(realtimestop)
        while ii<=length(realtimestart)
        eventvector((realtimestart(ii)+1):(realtimestop(ii)-1))=1;
        ii=ii+1;
        end
    end
