...
%         end
       ...
       if isempty(timestart)
        ...
        %Let's take care of the false positives while we're at it
        %let's say at least one of the neighbouring points have to be within one standard
        %deviation of the mean.
        lengthtimestart=length(timestart);
        lengthtimestop=length(timestop);
        timestopold=timestop;
        timestartold=timestart;
        
        
        
%         for ii=1:lengthtimestart
%            if donor(timestart(ii)-1)<(donorfit.mu+0.5*donorfit.sigma)||donor(timestart(ii))<(donorfit.mu+0.5*donorfit.sigma);
%               deletionvector(ii)=0;
%            else
%                deletionvector(ii)=1;
%            end
%         end
%         deletionvector=logical(deletionvector);
%         timestart(deletionvector)=[];
%         
%         clear deletionvector
%         for ii=1:lengthtimestop
%             if donor(timestop(ii)+1)<(donorfit.mu+0.5*donorfit.sigma)||donor(timestop(ii))<(donorfit.mu+0.5*donorfit.sigma);
%                  deletionvector(ii)=0;
%            else
%                deletionvector(ii)=1;
%             end
...
%         if lengthtimestart>2
%         for ii=2:lengthtimestart
%             
%             if nnz(donor(timestart(ii)-3:timestart(ii))<(donorfit.mu+1*donorfit.sigma))==0;
%                 deletionvector(ii)=1;
%             else
%                 deletionvector(ii)=0;
%             end
...
%         clear deletionvector
% 
% timestart=timestart(donor(timestart-2)<(donorfit.mu+1*donorfit.sigma));
% timestop=timestop(donor(timestop+2)<(donorfit.mu+1*donorfit.sigma));
