...
thresholdinput = str2num(input_ans{12});
...
if thresholdinput ~= 0,
    disp(['Threshold :' num2str(thresholdinput)]);
end
disp('------------------------');
...
acceptorfit             = fitdist(acceptor,'normal');
%then remake the fit
acceptorfit             = fitdist(acceptor(acceptor<(acceptorfit.mu+1.5*acceptorfit.sigma)),'normal');
        if thresholdinput == 0
%         donorfit             = fitdist(donor(donor<50),'normal'); 
        ...
        timestartA               = fretelements(startingpoint);
        fretelementsshiftedbwd  = circshift(fretelements,-1);
        fretdiff2               = fretelementsshiftedbwd-fretelements;
        endingpoint             = find(fretdiff2~=1);
        timestopA                = fretelements(endingpoint);
        %threslinea = threshold acceptor
        
        threslineA=threshold*ones(1,length(time));
        boundlineA=(acceptorfit.mu+1*acceptorfit.sigma)*ones(1,length(time));

% Do the same for the donor signal
...
%         donorfit             = fitdist(donor(donor<50),'normal'); 
        ...
        fretdiff                = fretdiff-1;
        startingpoint           = find(fretdiff~=0);
        timestartD               = fretelements(startingpoint);
        fretelementsshiftedbwd  = circshift(fretelements,-1);
        fretdiff2               = fretelementsshiftedbwd-fretelements;
        
        %threslined=threshold donor
        
        threslined=threshold*ones(1,length(time));
        boundlined=(donorfit.mu+1*donorfit.sigma)*ones(1,length(time));
        endingpoint             = find(fretdiff2~=1);
        timestopD                = fretelements(endingpoint);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   cj(1) = subplot(2,1,1);
   z=plot(time,donor,'g',time,acceptor,'r', time, donor+acceptor+offset,'k',time,threslined,time,boundlined,time,threslineA,time,boundlineA);
   set(z(4),'color','g','linestyle','--','linewidth',2);
   set(z(5),'color','g','linestyle','--');
   set(z(6),'color','r','linestyle','--','linewidth',2);
   set(z(7),'color','r','linestyle','--');
   title(fname);
   temp=axis;
   temp(1)=0;
   temp(2)=lengths;
...
%                fprintf(xoutfile,'%f\n',xCross(xcorr,2));
  ...
  % here we say that left click = acceptor selection and right click =
  % donor selection. 
  % first start with the acceptor
...
%   start looking for the starting and ending points
        ...
        ltimestartA=length(timestartA);
        ltimestopA=length(timestopA);

        for ii=1:ltimestartA
            while acceptor(timestartA(ii))>(acceptorfit.mu+1*acceptorfit.sigma)&&(timestartA(ii)-1)>0
                timestartA(ii)=timestartA(ii)-1;
            end
        end
        
        for ii=1:ltimestopA
            while acceptor(timestopA(ii))>(acceptorfit.mu+1*acceptorfit.sigma) && timestopA(ii)<length(acceptor)
                timestopA(ii)=timestopA(ii)+1;
            end
        end
        ...
        %you click <1 seconds then it gets rounded off to the first frame
        if L(1)<1./(int_time) && numel(L)~=0
            L(1)=1;
            timestartA(1) = 2;
        end
            
 % Now we do this again for the donor case
        
      
        ltimestartD=length(timestartD);
        ltimestopD=length(timestopD);

        for ii=1:ltimestartD
            while donor(timestartD(ii))>(donorfit.mu+1*donorfit.sigma)&&(timestartD(ii)-1)>0
                timestartD(ii)=timestartD(ii)-1;
            end
        end
        
        for ii=1:ltimestopD
            while donor(timestopD(ii))>(donorfit.mu+1*donorfit.sigma) && timestopD(ii)<length(donor)
                timestopD(ii)=timestopD(ii)+1;
            end
        end
        ...
        %%%%%% now we plot the figures, both acceptor and donor start- and
        %%%%%% stopvalues
        
   sizedD =floor(size(N)/2); %total number of regions
sizedA = floor(size(L)/2);
...
if (sizedA(2)~=0),
    ...
    for j=1:sizedA(2),
        ...
        startpointelemA=timestartA(timestartA>L(2*j-1)&timestartA<L(2*j));
        startpointtotA=[startpointtotA; startpointelemA];
        endpointelemA=timestopA(timestopA>L(2*j-1)&timestopA<L(2*j));
        endpointtotA=[endpointtotA; endpointelemA];
        if length(startpointtotA)~=length(endpointtotA)
            i=i-1;
    ...
    for j=1:sizedD(2),
        ...
        startpointelemD=timestartD(timestartD>L(2*j-1)&timestartD<L(2*j));
        startpointtotD=[startpointtotD; startpointelemD];
        endpointelemD=timestopA(timestopD>L(2*j-1)&timestopD<L(2*j));
        endpointtotD=[endpointtotD; endpointelemD];
        if length(startpointtotA)~=length(endpointtotA)
            i=i-1;
            display('retry the clicking');
        end
        ...
        figure(hdl1);
        subplot(2,1,1);
        h=plot(time,donor,'g',time,acceptor,'r', time, donor+acceptor+offset,'k',(startpointelemA-1)*int_time,acceptor(startpointelemA),'o',...
            (endpointelemA-1)*int_time,acceptor(endpointelemA),'o',(startpointelemD-1)*int_time,donor(startpointelemD),'o',(startpointelemD-1)*int_time,donor(startpointelemD),'o','linewidth',0.5);
        set(h(4),'linewidth',2,'MarkerEdgeColor','r');
        set(h(5),'linewidth',2,'MarkerEdgeColor','r');
        set(h(6),'linewidth',2,'MarkerEdgeColor','g');
        set(h(7),'linewidth',2,'MarkerEdgeColor','g');
        title(fname);
        temp=axis;
        temp(1)=0;
        temp(2)=lengths;
    ...
    nregions2=length(startpointelemD);
    ...
    if ans=='x',
        ...
        nregions=length(startpointtotA)
        for ii=1:nregions
            mtemp           = startpointtotA(ii):1:endpointtotA(ii);
            totalnumber     = 0;
            temp_time       = -1;
            for m=mtemp
                traceI      = traceI + 1;
...
if selection == 10,     % Auto-Threshold
   ...
   timestopA(end) = [];
    ...
    if isempty(timestartA)
        ...
        figure(hdl1);
        subplot(2,1,1);
        h=plot(time,donor,'g',time,acceptor,'r', time, donor+acceptor+offset,'k',(timestartA-1)*int_time,acceptor(timestartA),'o',...
            (timestopA-1)*int_time,acceptor(timestopA),'o','linewidth',2);
    ...
    if ans=='x',
        ...
        nregions=length(timestartA);       % number of selected regions
        melem=zeros(0,0);
        %               for ii = 1:nregions
        ...
        for j=1:nregions,
            ...
            mtemp = timestartA(j):1:timestopA(j);
            ...
            fprintf(Cy3_fid_traI,'%d\n',ave_Cy3/totalnumber);
            fprintf(Cy5_fid_traI,'%d\n',ave_Cy5/totalnumber);
            fprintf(FRET_fid_lengthI,'%d %d\n',totalnumber,timestartA(j));       %apparently the L(j) element gives the starting point of the mouseclick in the standard version
        end % end of sized(2)
    end
